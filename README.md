...
**定位對照（報告用）**：相機只在走道地磚上畫五個點（四角 A–D、正中心 O），不連線。點位用看得見的走道格線（約 X 170–260 cm、Y 270–405 cm），不用整張地圖外框，避免投到桌子／遠牆家具上。右側仍保留細格供數格，並用同一組五點與有人色格對外說明。人框加格子座標，例如 `ID1 (6,8)`。此分支預設開啟；`--no-floor-grid` 可關。
